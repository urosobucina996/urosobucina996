## Hi there 👋

<!--
**urosobucina996/urosobucina996** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

## 👋 About Me

I'm a software engineer who loves building things — not just at work, but everywhere.  
I enjoy automating tasks, creating games, practicing algorithms, and basically **turning ideas into real projects**.  
Coding is not only my job; it's something I genuinely enjoy doing.


### 🛠️ Skills
- Languages & Runtime: Python | JavaScript | Node.js | Bash
- Frameworks: Django | React
- Tools: Virtual Machines | Docker
- Databases: MySQL | PostgreSQL


# 🚀 Projects
### 🌟 **ci_cd_flask_app**
📌 Project to get random messages as response.  <br>
📌 Created for CI/CD learning and automation.  <br>
🔗 [Open Project](https://github.com/urosobucina996/ci_cd_flask_app)
<br>
### 🌟 **python_games**
📌 Make 2D game played in CMD, mimic old snake game. <br>
📌 Just like on Tetris. <br>
🔗 [Open Project](https://github.com/urosobucina996/python_games)
<br>
### 🌟 **space_impact**
📌 Own verison of Space Impact game from old Nokia phones. <br>
📌 Made with React + Phaser <br>
🔗 [Open Project](https://github.com/urosobucina996/space_impact)
<br>
### 🌟 **bash-scripts**
📌 Bash scripts to automate upload files. <br>
🔗 [Open Project](https://github.com/urosobucina996/bash-scripts/tree/main/upload-script)
<br>
### 🌟 **pandemicStatistic**
📌 Web app that scrapes and represent statistic data about Covid(made in 2022). <br>
🔗 [Open Project](https://github.com/urosobucina996/pandemicStatistic) <br>
🔗 **Site:**  `https://pandemic-report-urosobucina996.vercel.app/report`
<br>
### 🌟 **webScraper**
📌 Web scraper for electric guitars <br>
🔗 [Open Project](https://github.com/urosobucina996/webScraper)
<br>
### 🌟 **nodeREST**
📌  Web scraper for guitars with Node <br>
🔗 [Open Project](https://github.com/urosobucina996/nodeREST)
<br>
